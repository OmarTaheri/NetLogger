...
export async function analyzeVisitor(visitor: Record<string, any>, userId: number): Promise<Record<string, any>> {
  ...
  const uniquenessScore = await computeUniquenessScore(visitor, userId);
