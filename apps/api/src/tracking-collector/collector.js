...
function collectDevice() {
  ...
  var asyncCollection = Promise.all([
  ...
  // Fingerprinting enriches the record, but it must never block GPS collection
  // or navigation when a browser API stalls (media devices and WebRTC are the
  // most common offenders in privacy-focused and embedded browsers).
  var collectionDeadline = new Promise(function(resolve) {
    setTimeout(function() {
      Object.assign(deviceInfo, BehaviorTracker.summarize());
      resolve(deviceInfo);
    }, 4000);
  ...
  return Promise.race([asyncCollection, collectionDeadline]);
}
