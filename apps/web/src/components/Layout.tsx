...
export default function Layout({ displayName, onLogout, guestMode = false, children }: LayoutProps) {
  const [sidebarOpen, setSidebarOpen] = useState(false);
  useKeyboardShortcuts(!guestMode);
  ...
  return (
    <div className="flex flex-col h-screen">
      ...
      <header className="h-10 bg-hud-bg border-b border-hud-border flex items-center justify-between px-4 flex-shrink-0 opacity-0 animate-hud-blink">
        <div className="flex items-center gap-3">
          ...
          <span className="font-mono text-hud-sm uppercase tracking-widest text-hud-text-dim hidden sm:inline">
            NETLOGGER <span className="text-hud-accent">//</span> VISITOR INTELLIGENCE
          </span>
        ...
        <div className="flex items-center gap-4">
          <div className="hidden sm:flex items-center gap-3">
            ...
            <span className="flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 bg-hud-green rounded-full" />
              <span className="text-hud-xs uppercase font-mono text-hud-text-muted">{guestMode ? 'GUEST SESSION' : 'WS CONNECTED'}</span>
            </span>
          </div>
          <div className="w-px h-4 bg-hud-border hidden sm:block" />
          <HudClock />
          <div className="w-px h-4 bg-hud-border" />
          <span className="font-mono text-hud-xs uppercase text-hud-text-dim hidden sm:inline">{displayName}</span>
          {guestMode ? (
            <div className="flex items-center gap-3">
              <Link to="/login" className="font-mono text-hud-xs uppercase text-hud-accent hover:text-hud-accent/80">LOG IN</Link>
              <Link to="/signup" className="hidden sm:inline-flex border border-hud-accent px-2.5 py-1.5 font-mono text-hud-xs uppercase text-hud-accent hover:bg-hud-accent/10">CREATE ACCOUNT</Link>
            </div>
          ) : (
            <button
              onClick={onLogout}
      ...
      <div className="flex flex-1 min-h-0">
        ...
        `}>
          {navItems.map((item) => {
            const locked = guestMode && item.to !== '/create';
            if (locked) {
              return (
                <button
                  key={item.to}
                  type="button"
                  disabled
                  aria-label={`${item.label} — account required`}
                  className="relative w-10 h-10 flex items-center justify-center text-hud-text-muted/35 cursor-not-allowed group"
                >
                  {item.icon}
                  <span className="absolute right-0 top-0 text-hud-accent"><LockIcon /></span>
                  <span className="absolute left-full ml-2 px-2 py-1 bg-hud-surface border border-hud-border font-mono text-hud-xs uppercase text-hud-text whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50">
                    {item.label} // ACCOUNT REQUIRED
                  </span>
                </button>
              );
            }
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `relative w-10 h-10 flex items-center justify-center transition-colors group ${
                    isActive ? 'text-hud-accent' : 'text-hud-text-muted hover:text-hud-text-dim'
                  }`
                }
              >
                {({ isActive }) => (
                  <>
                    {isActive && <div className="absolute left-0 top-1 bottom-1 w-0.5 bg-hud-accent" />}
                    {item.icon}
        ...
        {/* Main content area */}
        <main id="dashboard-scroll-root" className="flex-1 overflow-auto bg-hud-bg-alt hud-grid-overlay hud-scanline-overlay">
